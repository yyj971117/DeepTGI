...
def prepare():
    ...
    extra_gene=pd.read_csv('~/DeepTGI-main/DeepTGI/dataset/scRNA-Seq/mHSC-E/ExpressionDataOrderedfl.csv')
    ...
    pair_label = pd.read_csv('~/DeepTGI-main/DeepTGI/dataset/scRNA-Seq/mHSC-E/GenePairListmHSC-E-Non-1000-GPL.csv')
...
def BERT_train(model, x_train, y_train, x_test, y_test, event_num):
    ...
    for epoch in range(epo_num):
        ...
        for batch_idx, data in enumerate(train_loader, 0):
            ...
            #print('target_a',targets_a.shape)
            #print('target_b',targets_b.shape)
            #print('X',X.shape)
            #print('X_AE1',X_AE1.shape) 
            loss = lam * my_loss(X, targets_a, inputs, X_AE1) + (1 - lam) * my_loss(X, targets_b, inputs,
                                                                                                  X_AE1)
...
def cross_val(feature, label, event_num):
    ...
    for train_index, test_index in skf.split(feature, label):
        mat_GENIE3=pd.read_csv('~/DeepTGI-main/DeepTGI/test_R/pred_nets/GENIE3/GENIE3-mHSC-E.csv',sep=' ')
        mat_PPCOR=pd.read_csv('~/DeepTGI-main/DeepTGI/test_R/pred_nets/PPCOR/PPCOR-mHSC-E.csv',sep=' ')
        mat_PIDC=pd.read_csv('~/DeepTGI-main/DeepTGI/test_R/pred_nets/PIDC/PIDC-mHSC-E.csv',sep=' ')
        model = BERT(len(feature[0]), bert_n_heads, bert_n_layers, event_num)
        print('test_index',test_index) 
        label_all=np.array(pd.read_csv('/DeepTGI-main/DeepTGI/dataset/scRNA-Seq/mHSC-E/GenePairListmHSC-E-Non-1000-GPL.csv'))

        np.random.seed(seed)
        np.random.shuffle(label_all)
        label_test=pd.DataFrame(label_all[test_index])
        label_test.columns=['tf','gene','label','score']
        #label_test.to_csv('~/test_R/test_tf_gene_mHSC_L_1000/test_label%d.csv'%od,index=False)
    
        #np.save('/home/zhongle/zhongle/DGRNS/Dataset/scRNA-Seq/mHSC-L/index_1000/fold%d/train_index.npy'%od,train_index)
        #np.save('/home/zhongle/zhongle/DGRNS/Dataset/scRNA-Seq/mHSC-L/index_1000/fold%d/test_index.npy'%od,test_index)
        ...
        for i in range(len(test_index)):
            tf,gene=label_test.iloc[i,0],label_test.iloc[i,1]
            GENIE3.append(mat_GENIE3.loc[tf,gene])
            PPCOR.append(mat_PPCOR.loc[tf,gene])
            PIDC.append(mat_PIDC.loc[tf,gene])

        result_,pred_one=(cal_metrics(y_test,F.sigmoid(torch.Tensor(pred_score))))
        _,pred_one_GENIE3=cal_metrics(y_test,GENIE3)
        _,pred_one_PPCOR=cal_metrics(y_test,PPCOR)
        _,pred_one_PIDC=cal_metrics(y_test,PIDC)

        label_test['DeepTGI']=pred_one
        label_test['GENIE3']=pred_one_GENIE3
        label_test['PIDC']=pred_one_PIDC
        label_test['PPCOR']=pred_one_PPCOR

        test_pairs_all=pd.concat([label_test,test_pairs_all])
        #print(test_pairs_all)
        #test_pairs_all.to_csv('~/test_R/test_tf_gene_mHSC_E_1000/label_DeepTGI.csv',index=False)
