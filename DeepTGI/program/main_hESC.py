...
def prepare():
    ...
    extra_gene=pd.read_csv('~/DeepTGI-main/DeepTGI/dataset/human/hHep/expr1.csv')
    print(extra_gene)
    ...
    pair_label = pd.read_csv('~/DeepTGI-main/DeepTGI/dataset/human/hHep/label1.csv')
    #pair_label = get_low(pair_label)
    pair_label.columns=['tf','gene','label']
...
def cross_val(feature, label, event_num):
    ...
    od=0
    GENIE3_pred = np.zeros(6)
    PIDC_pred = np.zeros(6)
    PPCOR_pred = np.zeros(6)
    for train_index, test_index in skf.split(feature, label):
        
        mat_GENIE3=pd.read_csv('~/DeepTGI-main/DeepTGI/test_R/pred_nets/GENIE3/GENIE-hHep.csv',sep=' ')
        mat_PPCOR=pd.read_csv('~/DeepTGI-main/DeepTGI/test_R/pred_nets/PPCOR/PPCOR-hHep.csv',sep=' ')
        mat_PIDC=pd.read_csv('~/DeepTGI-main/DeepTGI/test_R/pred_nets/PIDC/PIDC-hHep.csv',sep=' ')
        
        model = BERT(len(feature[0]), bert_n_heads, bert_n_layers, event_num)
        
        label_all=np.array(pd.read_csv('~/DeepTGI-main/DeepTGI/dataset/human/hHep/label1.csv'))

        np.random.seed(seed)
        np.random.shuffle(label_all)
        label_test=pd.DataFrame(label_all[test_index])
        label_test.columns=['tf','gene','label']
        #label_test.to_csv('~/test_R/test_tf_gene_mHSC_L_1000/test_label%d.csv'%od,index=False)
    
        #np.save('/home/zhongle/zhongle/human/hESC/fold/fold%d/train_index.npy'%od,train_index)
        #np.save('/home/zhongle/zhongle/human/hESC/fold/fold%d/test_index.npy'%od,test_index)
        ...
        label_test.to_csv('~/test_R/test_tf_gene_hESC/test_label%d.csv'%od,index=False)
        GENIE3=[]
        PPCOR=[]
        PIDC=[]
        od = od+1
        ...
        pred_GENIE3,pred_one_GENIE3=cal_metrics(y_test,GENIE3)
        pred_PPCOR,pred_one_PPCOR=cal_metrics(y_test,PPCOR)
        pred_PIDC,pred_one_PIDC=cal_metrics(y_test,PIDC)
           
        GENIE3_pred = GENIE3_pred + np.array(pred_GENIE3)
        PPCOR_pred =   PPCOR_pred + np.array(pred_PPCOR)
        PIDC_pred = PIDC_pred + np.array(pred_PIDC)
